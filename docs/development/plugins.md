...
The Hook command differs from Codex because ZCode supports neither `commandWindows` nor `additionalContextLimit`. `hooks/hooks.json` references the polyglot `hooks/run-hook.cmd` through `${ZCODE_PLUGIN_ROOT}`, the host's native plugin-root variable (the Claude-compatible `${CLAUDE_PLUGIN_ROOT}` token expands to the same path); on Windows the batch portion locates Git Bash and runs the extensionless `hooks/session-start` script, on Unix the same file runs as a bash script directly. The `session-start` script wraps `lore hook zcode` and prints `{"continue":true}` when the CLI is missing or exits non-zero, so the host session is never blocked. The Catalog budget is enforced by the shared CLI renderer (4000 characters), not by a host field.

...
- 新增 ZCode Plugin `plugins/lorelum-zcode/`：`.zcode-plugin/plugin.json` manifest（插件名 `lorelum-zcode`）、`commands/lore.md`（`/lore` 命令，接受可选自然语言问题）、`skills/lorelum/SKILL.md` 与 recovery reference、`hooks/hooks.json`（SessionStart，经 `${CLAUDE_PLUGIN_ROOT}` 调用跨平台包装脚本执行 `lore hook zcode`）、品牌图标、README 与 Plugin 配置测试。

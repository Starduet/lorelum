...
- ZCode 不支持 `commandWindows` 与 `additionalContextLimit`；ZCode Plugin 通过 `${CLAUDE_PLUGIN_ROOT}/hooks/run-hook.cmd` polyglot 包装脚本跨平台调用无扩展名的 `hooks/session-start`，后者包装 `lore hook zcode` 并在 CLI 缺失或非零退出时兜底输出 `{"continue":true}`。上下文预算由 CLI 渲染器的 4000 字符上限保证，与 codex 共享同一实现。
